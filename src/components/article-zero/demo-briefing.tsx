...
export function DemoBriefing({ onOpenConstitution, onRunGuidedDemo }: DemoBriefingProps) {
  return (
    <section className="az-briefing" aria-labelledby="briefing-title">
      <div className="az-briefing-kicker"><span className="az-status-dot" aria-hidden="true" />Synthetic hospital scenario / clean workspace</div>
      ...
      <div className="az-briefing-grid">
        <div className="az-briefing-story">
          ...
          <p>A fictional responder claims a patient is in immediate danger. The command center will expose the legacy gap, amend the clause, and prove the safer rule with replayable tests.</p>
        </div>
        <div className="az-briefing-callout">
          <span className="az-callout-number">01</span>
          <p>All records, identities, and outcomes in this demo are synthetic. Article Zero is not a clinical system.</p>
        </div>
      </div>
      <div className="az-briefing-actions">
        <button className="az-button az-button-primary" type="button" onClick={onOpenConstitution}>Open the Constitution <ArrowRight size={16} aria-hidden="true" /></button>
        <button className="az-button az-button-secondary" type="button" onClick={onRunGuidedDemo}><Play size={15} aria-hidden="true" /> Run guided demo</button>
      </div>
    </section>
  );
}
