...
export function CompletionSummary({ onOpenAudit, onExport, onReturnHome, onStartAnotherSimulation, isExporting }: CompletionSummaryProps) {
  ...
  return <section className="az-completion-summary" aria-labelledby="completion-title">
    ...
    <div className="az-completion-grid">
      ...
      <section className="az-review-card"><p className="az-eyebrow">Latest tests</p><h2>{testRun === null ? "No tests produced" : `${passedTests}/${testRun.results.length} tests passed`}</h2>{testRun !== null ? <details className="az-completion-tests"><summary>Show test result details</summary><ul className="az-test-results">{testRun.results.map((result) => <li key={result.testCaseId} className={result.passed ? "az-test-pass" : "az-test-fail"}><strong>{result.passed ? "Pass" : "Fail"}</strong><span>{SEEDED_REGRESSION_CASES.find((testCase) => testCase.id === result.testCaseId)?.name ?? "Named policy test"}</span><small>{result.failureDetail ?? "Expected behavior observed."}</small></li>)}</ul></details> : <p className="az-help-text">Run the deterministic suite before activating a version.</p>}</section>
