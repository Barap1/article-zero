...
export function AuditDrawer({ events, onClose, onExport }: AuditDrawerProps) {
  ...
  return <aside className="az-audit-drawer" role="dialog" aria-modal="true" aria-labelledby="audit-drawer-title">
    ...
    <label className="az-field-label" htmlFor="audit-filter">Filter audit events<select id="audit-filter" value={filter} onChange={(event) => setSelectedFilter(event.target.value)}><option value={allEvents}>All event types</option>{eventTypes.map((type) => <option key={type} value={type}>{type}</option>)}</select></label>
    <ol className="az-audit-list">{visibleEvents.map((event) => <li key={event.id}><div><strong>{event.type}</strong><span>{event.timestamp} · {event.source} · {event.constitutionVersionId ?? "no version"}</span><p>{event.detail}</p></div><details><summary>Typed detail</summary><pre>{JSON.stringify({ relatedIds: event.relatedIds, integrityHash: event.integrityHash }, null, 2)}</pre></details></li>)}</ol>
