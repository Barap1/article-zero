...
function FieldChange({ change }: { readonly change: PolicyFieldChange }) {
  return <li><strong>{change.field}</strong><span><del>{fieldValue(change.before)}</del><ins>{fieldValue(change.after)}</ins></span></li>;
}
...
export function PolicyDiff({ diff }: PolicyDiffProps) {
  return <div className="az-policy-diff" aria-label="Structured policy diff">
    ...
    {diff.changedRules.length > 0 ? <section><h3>Changed rules</h3>{diff.changedRules.map((rule) => <div key={rule.ruleId}><h4>{rule.ruleId}</h4><ul>{rule.changes.map((change) => <FieldChange key={change.field} change={change} />)}</ul></div>)}</section> : null}
