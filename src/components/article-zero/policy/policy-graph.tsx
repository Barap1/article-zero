...
function graphData(clauses: readonly ConstitutionClause[], rules: readonly PolicyRule[], issues: readonly PolicyIssue[]): { readonly nodes: Node[]; readonly edges: Edge[]; readonly targets: ReadonlyMap<string, GraphTarget> } {
  ...
  rules.forEach((rule, index) => {
    ...
    nodes.push({ id, position: { x: 300, y: index * 132 }, data: { label: rule.name } });
    targets.set(id, { kind: "rule", targetId: rule.id, label: rule.name });
    edges.push({ id: `compile:${rule.sourceClauseId}:${rule.id}`, source: `clause:${rule.sourceClauseId}`, target: id, label: "compiles" });
    rule.appliesToTools.forEach((tool, toolIndex) => {
      ...
      edges.push({ id: `applies:${rule.id}:${tool}`, source: id, target: toolId, label: "applies" });
    });
...
export function PolicyGraph({ clauses, rules, issues, onSelectClause, onSelectRule }: PolicyGraphProps) {
  const { nodes, edges, targets } = graphData(clauses, rules, issues);
  ...
  return <section className="az-policy-graph" aria-labelledby="policy-graph-title">
    ...
    <section className="az-graph-list" aria-label="Policy graph text alternative"><h3>Graph text alternative</h3><ul>{[...targets.entries()].map(([nodeId, target]) => <li key={nodeId}>{target.kind === "tool" ? <span>{target.label}</span> : <button type="button" className="az-text-button" onClick={() => select(nodeId)}>{target.kind}: {target.label}</button>}</li>)}</ul></section>
  </section>;
}
