...
import { LEGACY_POLICY_BUNDLE, PRE_REPAIR_POLICY_BUNDLE, SEED_CLAUSES } from "../hospital/fixtures/constitution";
...
export function createSeedWorkspace(fixedNow: Date = new Date("2026-07-13T12:00:00.000Z")): WorkspaceState {
  ...
  const draft = version({ id: "version.draft-v1-1", label: "v1.1 Draft", status: "DRAFT", createdAt: timestamp, activatedAt: null, parentVersionId: legacy.id, clauses: [...SEED_CLAUSES], policyBundle: { ...PRE_REPAIR_POLICY_BUNDLE, rules: [...PRE_REPAIR_POLICY_BUNDLE.rules] }, bundleHash: "seed-hash-draft-v1-1", activationTestRunId: null, acknowledgedIssueIds: [], changeSummary: "Editable child of the legacy baseline." });
